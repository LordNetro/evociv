import { Grid, type TileData } from './grid';
...
export class Engine {
	...
	updateSnapshot(data: unknown): void {
		...
		this.grid.updateTiles(((data as Record<string, unknown>)?.tiles as TileData[]) ?? []);
	}

	// ── Input handling ──────────────────────────────────

	private setupInput(): void {
		const drag = { active: false, sx: 0, sy: 0, camX: 0, camY: 0, lx: 0, ly: 0 };
