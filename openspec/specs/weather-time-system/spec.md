# Spec: weather-time-system

> Synced from delta `openspec/changes/weather-time-system/specs/weather-time-system/spec.md` (2026-05-03)

## Purpose

Define day/night cycle and weather simulation. Weather affects resource regeneration, agent attributes, status effects, emotions, and visibility. Shelters mitigate exposure. Weather and time data is injected into LLM prompts for more grounded agent decision-making.

## Dependencies

- **Depends on**: simulation-engine, data-driven-definitions (DEFINITIONS singleton, YAML loader)
- **Depends on**: status-effect-system (StatusEffectManager for effect aggregation)
- **Depends on**: emotion-system (EmotionManager for weather-triggered emotions)
- **Depends on**: structures (shelter_protection field for weather mitigation)

## Capabilities

### capability: weather-time-system
**Depends on**: simulation-engine, data-driven-definitions, status-effect-system, emotion-system, structures
...
##### Scenario: Night resource regen penalty
...
##### Scenario: Emotion triggers on weather change
