<script lang="ts">
  ...
  function getBarColor(stat: string): string {
    switch (stat) {
      case 'hunger': return '#f44336';
      case 'thirst': return '#2196F3';
  ...
  <div class="inspector">
    ...
    <details class="section" open>
      ...
      <div class="section-body">
        {#each ['hunger', 'thirst', 'energy', 'health'] as stat}
          {@const value = Math.max(0, Math.min(100, Math.round((agent[stat as keyof AgentData] as number) ?? 0)))}
          <div class="stat-row">
            <span class="stat-label">{stat.charAt(0).toUpperCase() + stat.slice(1)}</span>
            <div class="stat-bar-bg">
              <div class="stat-bar-fill" style="width: {value}%; background-color: {getBarColor(stat)};"></div>
            </div>
    ...
    <details class="section">
      ...
      <div class="section-body">
        <div class="kv-row"><span class="kv-key">Role</span><span class="kv-value">{agent.role ?? '—'}</span></div>
        <div class="kv-row"><span class="kv-key">Sex</span><span class="kv-value">{agent.sex ?? '—'}</span></div>
        <div class="kv-row"><span class="kv-key">Age</span><span class="kv-value">{agent.age ?? 0} / {agent.max_age ?? 0} ticks</span></div>
      </div>
    </details>
    ...
    <details class="section">
      ...
      <div class="section-body">
        <div class="kv-row"><span class="kv-key">Strength</span><span class="kv-value">{agent.strength ?? 0}</span></div>
        <div class="kv-row"><span class="kv-key">Intelligence</span><span class="kv-value">{agent.intelligence ?? 0}</span></div>
        <div class="kv-row"><span class="kv-key">Sociability</span><span class="kv-value">{agent.sociability ?? 0}</span></div>
        <div class="kv-row"><span class="kv-key">Speed</span><span class="kv-value">{agent.speed ?? 0}</span></div>
      </div>
    </details>
    ...
    <details class="section">
      ...
      <div class="section-body">
        {#if agent.inventory && Object.keys(agent.inventory).length > 0}
          {#each Object.entries(agent.inventory) as [item, count]}
            <div class="kv-row">
      ...
      <div class="section-body">
        {#if agent.monologue_history && agent.monologue_history.length > 0}
          <ul class="monologue-list">
            {#each agent.monologue_history.slice(-5) as thought}
              <li>{thought}</li>
            {/each}
          </ul>
        {:else}
